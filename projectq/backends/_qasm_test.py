...
from projectq.ops import (X, NOT, Y, Z, T, Tdagger, S, Sdagger, H, Ph, R, Rx,
                          Ry, Rz, Swap, Allocate, Deallocate, Measure, Barrier,
                          Entangle, Command, All)
...
@pytest.mark.parametrize("gate, is_available",
                         [(X, True), (Y, True), (Z, True), (T, True),
                          (Tdagger, True), (S, True), (Sdagger, True),
                          (Allocate, True), (Deallocate, True), (Measure, True),
                          (NOT, True), (Rx(0.5), True), (Ry(0.5), True),
                          (Rz(0.5), True), (R(0.5), True), (Ph(0.5), True),
...
@pytest.mark.parametrize("gate, is_available",
                         [(H, True), (X, True), (NOT, True), (Y, True),
                          (Z, True), (Rz(0.5), True), (R(0.5), True),
...
@pytest.mark.parametrize("gate, is_available",
                         [(X, True), (NOT, True), (Y, False), (Z, False),
                          (Rz(0.5), False), (R(0.5), False), (Rx(0.5), False),
                          (Ry(0.5), False)],
                         ids=lambda l: str(l))
...
def test_qasm_test_qasm_single_qubit_gates():
    ...
    assert qasm[2:-1] == [
        'qubit q0;', 'bit c0;', 'h q0;', 's q0;', 't q0;', 'sdg q0;', 'tdg q0;',
        'x q0;', 'y q0;', 'z q0;', 'u1(0.5) q0;', 'rx(0.5) q0;', 'ry(0.5) q0;',
        'rz(0.5) q0;', 'u1(-0.25) q0;', 'x q0;', 'c0 = measure q0;'
    ]
...
def test_qasm_test_qasm_single_qubit_gates_control():
    ...
    # Note: ignoring header and footer for comparison
    assert qasm[2:-1] == [
        'qubit q0;', 'bit c0;', 'qubit q1;', 'bit c1;', 'ch q1,q0;',
        'cx q1,q0;', 'cy q1,q0;', 'cz q1,q0;', 'cx q1,q0;', 'cu1(0.5) q1,q0;',
        'crz(0.5) q1,q0;', 'cu1(-0.25) q1,q0;', 'c0 = measure q0;',
        'c1 = measure q1;'
    ]
...
def test_qasm_no_collate():
    ...
    eng = MainEngine(backend=OpenQASMBackend(collate=False,
                                             collate_callback=_process,
                                             qubit_id_mapping_redux=False),
                     engine_list=[])
    qubit = eng.allocate_qubit()
    ctrls = eng.allocate_qureg(2)
...
def test_qasm_name_callback():
    ...
    eng = MainEngine(backend=OpenQASMBackend(qubit_callback=_qubit,
                                             bit_callback=_bit),
                     engine_list=[])
    ...
    qasm = eng.backend.qasm
    assert qasm[2:] == [
        'qubit qubit_0;', 'bit classical_bit_0;',
        'classical_bit_0 = measure qubit_0;'
    ]
