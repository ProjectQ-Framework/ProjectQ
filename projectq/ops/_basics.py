...
from projectq.types import BasicQubit, Qureg
...
class BasicGate(object):
    ...
    def __and__(self, controls):
        if isinstance(controls, BasicQubit):
            controls = Qureg([controls])
        if len(controls) == 0:
            return self
        return _GateWithIncludedControls(self, controls)
...
class _GateWithIncludedControls(BasicGate):
    """
    A wrapper class that adds given controls to commands generated by a given
    gate.
    """
    ...
    def __init__(self, gate, controls):
        """
        Args:
            gate (BasicGate): The gate to apply conditionally.
            controls (Qureg): The qubits to condition the gate on.
        """
        BasicGate.__init__(self)
        self._gate = gate
        self._controls = controls
        if isinstance(gate, _GateWithIncludedControls):
            self._gate = gate._gate
            self._controls = self._controls + gate._controls

    def get_inverse(self):
        return _GateWithIncludedControls(self._gate.get_inverse(),
                                         self._controls)
    ...
    def __eq__(self, other):
        return (isinstance(other, _GateWithIncludedControls) and
